...
class State(object):
    ...
    def cost(self, state):
        ...
        return math.sqrt(math.pow((self.x - state.x), 2) +     # 可能需要修改一下cost的计算方式
...
class Dstar(object):
    ...
    def modify_cost(self, x):
        if x.t == "close":  # 是以一个openlist，通过parent递推整条路径上的cost
            ...
            # print("是否相等:",x.parent.h + x.cost(x.parent) == x.h)
        return self.get_kmin()

    def run(self, start, end):
        # self.open_list.add(end) 根据评论修改
        self.insert(end,0)		
        while True:
            self.process_state()
            if start.t == "close":
        ...
        self.map.print_map()
        tmp = start # 起始点不变
        self.map.set_obstacle([(9,1),(9,2),(9, 3), (9, 4), (9, 5), (9, 6), (9, 8),(9,9),(9,10),(9,11)]) # 障碍物发生变化
        '''
    ...
    def modify(self, state):
        ...
        :return:
        '''
        self.modify_cost(state) # robot发现障碍时所在的state
        while True:
            k_min = self.process_state()
            if k_min >= state.h:  # 新的路径构建完成
                break
